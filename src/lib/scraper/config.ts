...
export async function ensureScraperSourceConfigs(
  ...
  for (const src of CLINIC_SOURCES) {
    ...
    // Per-source parserType takes precedence only for new rows; existing rows
    // keep their operator-chosen type unless `forceParserType` is passed.
    // This lets the curated CLINIC_SOURCES list declare "real_html" for sources
    // backed by real public price lists while still allowing operators to
    // override via the admin UI.
    const declaredParserType =
      opts.forceParserType ?? src.parserType ?? "simulated";
