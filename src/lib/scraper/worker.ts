...
// Import real scraper modules for their registration side effect. The
// `SimulatedScraper` is already registered by `registry.ts` (imported above);
// this line adds `RealHtmlScraper` to the registry under the "real_html" key
// so sources with `parserType: "real_html"` resolve to a live HTTP fetcher
// (with graceful fallback to the deterministic generator on failure).
import "./scrapers/real-html";
import type {
  IngestionJobReport,
  IngestionOptions,
  SourceRunOutcome,
